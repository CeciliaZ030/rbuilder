...
use reth::{builder::NodeConfig, revm::cached::CachedReads};
    ...
    pub fn create_relays(&self, l1_client: Option<HttpClient>) -> eyre::Result<Vec<MevBoostRelay>> {
        ...
        for relay_name in effective_enabled_relays.iter() {
            match relay_configs.get(relay_name) {
                Some(relay_config) => match MevBoostRelay::from_config(relay_config, l1_client.clone(), proposal_cache.clone()) {
                    Ok(relay) => {
                        info!(
